...
::: 注意
受 `mirai-console-loader` 的影响，`mirai-api-http` 请下载 `v2.1.1` 及以下版本
:::
...
从 [mirai-api-http release]() 下载最新的 `.jar` 文件.
...
安装好 `mirai-api-http` 后, 在 `config/net.mamoe.mirai-api-http` 目录下, 你会找到 `setting.yml` 文件.
...
1. 用 `TxCaptchaHelper` 滑块验证助手来进行验证.
...
进入  `bots/<你的QQ号>` 下面, 将导出的 `device.json` 复制进入.
...
<code-group>
  ...
  services:
    mcl:
      ...
      volumes: # 挂载目录
        - ./mcl/bots:/app/bots
        - ./mcl/config/Console/AutoLogin.yml:/app/config/Console/AutoLogin.yml
        - ./mcl/config/net.mamoe.mirai-api-http/setting.yml:/app/config/net.mamoe.mirai-api-http/setting.yml
  ```
  </code-block>
</code-group>
